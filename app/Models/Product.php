...
class Product extends ZohoModel
{
    ...
    {
        if ($this->Vendor_Name)
            return (new Vendor())->newBuilder()->find($this->Vendor_Name["id"]);
    }
    ...
    public function location()
    {
        if ($this->Plataforma_API)
            return match ($this->Plataforma_API) {
                'Systrack' => (new Systrack())->newBuilder()->find($this->Clave_API)->getLocation(),
                'Navixy' => (new Navixy())->newBuilder()->find($this->Clave_API)->getLocation(),
            };
    }
}
